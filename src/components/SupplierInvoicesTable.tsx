...
import { SUPPLIER_INVOICE_STATUSES } from "@/lib/lists";
import { Card } from "./ui";
...
export default function SupplierInvoicesTable({
  ...
  return (
    <FilterableList
      items={invoices}
      searchPlaceholder="Search by supplier, invoice ID, service…"
      searchText={(inv) =>
        `${inv.invoice_id} ${inv.supplier} ${inv.service_type} ${inv.payment_status} ${inv.supplier_invoice_no ?? ""}`
      }
      statusOptions={SUPPLIER_INVOICE_STATUSES.map((s) => ({
        value: s,
        label: s,
      }))}
